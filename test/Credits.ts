...
describe("Credits", () => {
    ...
    before("Initialization", async function() {
        ...
        credits = await Credits.deploy(owner.address, metadataURI, contractMetadataURI, royaltyBps);
    ...
    describe("Roles", () => {
        it("Sets admin", async () => {
            await expect(credits.setAdmin(account1.address, true))
                .emit(credits, "RoleAssignment")
                .withArgs(
                    account1.address,
                    0,
                    false
            ...
            expect(await credits.isAdmin(account1.address)).is.true;
            expect(await credits.isAdmin(account2.address)).is.false;

            await expect(credits.connect(account1).setAdmin(account2.address, true)).reverted;
            await expect(credits.connect(account2).setAdmin(account2.address, true)).reverted;

            await expect(credits.setAdmin(account1.address, false))
                .emit(credits, "RoleAssignment")
                .withArgs(
                    account1.address,
                    0,
            ...
            expect(await credits.isAdmin(account1.address)).is.false;

            // account1 is admin.
            await credits.setAdmin(account1.address, true);
        });

        it("Sets manager", async () => {
            await expect(credits.setManager(account1.address, true))
                .emit(credits, "RoleAssignment")
                .withArgs(
                    account1.address,
                    1,
            ...
            expect(await credits.isManager(account1.address)).is.true;
            expect(await credits.isManager(account2.address)).is.false;

            await expect(credits.connect(account1).setManager(account2.address, true)).reverted;
            await expect(credits.connect(account2).setManager(account2.address, true)).reverted;

            await expect(credits.setManager(account1.address, false))
                .emit(credits, "RoleAssignment")
                .withArgs(
                    account1.address,
                    1,
            ...
            expect(await credits.isManager(account1.address)).is.false;

            // account2 is manager.
            await credits.setManager(account2.address, true);
        });

        it("Sets owner", async () => {
            expect(await credits.owner()).equal(owner.address);

            await expect(credits.connect(account1).setOwner(account1.address)).reverted;
            await expect(credits.connect(account2).setOwner(account2.address)).reverted;

            await expect(credits.setOwner(account1.address))
                .emit(credits, "RoleAssignment")
                .withArgs(
                    account1.address,
                    2,
            ...
            expect(await credits.owner()).equal(account1.address);

            await credits.connect(account1).setOwner(owner.address);
        });
    });
    ...
    describe("Contract metadata", () => {
       it("Sets contract uri", async () => {
           ...
           await expect(credits.connect(account1).setContractURI(newUri)).reverted;
    ...
    describe("Bank", () => {
        it("Withdraws", async () => {
            ...
            await expect(credits.connect(account1).withdraw(token.address, account1.address, 1)).reverted;
    ...
    describe("Projects", () => {
        it("Creates project", async () => {
            await expect(credits.connect(account1).createProject()).reverted;

            await expect(credits.connect(account2).createProject())
        ...
        it("Creates credits for project", async () => {
            ...
            await expect(credits.connect(account1).newCreditsForProject(projectId, amount, account1.address)).reverted;
            ...
            expect(await credits.connect(account2).newCreditsForProject(projectId, amount, account2.address))
                .emit(credits, "NewCreditsReleased")
                .withArgs(
                    creditIds.length + 1,
                    projectId,
    ...
    describe("ERC-1155", () => {
        ...
        it("Updates metadata uri", async () => {
            ...
            await expect(credits.connect(account1).updateMetadataUri(newUri)).reverted;
    ...
    describe("Fundings", () => {
        it("Opens phase", async () => {
            ...
            await credits.connect(account2).openPhase(
                projectIds[1],
                amount,
                price,
                token.address
        ...
        it("Closes phase", async () => {
            await expect(credits.connect(account1).closePhase(projectIds[0], false)).reverted;

            await expect(credits.connect(account2).closePhase(projectIds[0], true))
                .emit(credits, "PhaseAction")
                .withArgs(
                    creditIds.length - 1,
                    false,
